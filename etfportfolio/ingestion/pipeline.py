...
from etfportfolio.ingestion import details, products, session, themes
...
async def _run_full(product_ids: str | None, limit: int | None, force: bool) -> None:
    with connect() as conn:
        ...
        console.info("=== Phase 2: Session validation ===")
        client, account_id = await session.ensure_session()
        console.info(f"Session OK. Active account: {account_id}")

        try:
            console.info("=== Phase 3: Theme taxonomy sync ===")
            try:
            ...
            console.info("=== Phase 4: Product details ===")
            target_ids = products.resolve_target_ids(conn, product_ids, limit)
            await _run_details_phase(conn, client, account_id, target_ids, force)
        finally:
            await client.aclose()
...
class Ingest:
    ...
    All logic lives in the domain modules (session, products, themes,
    details) — this class only wires phases together and reports progress.
    """

    def __call__(self, product_ids: str | None = None, limit: int | None = None, force: bool = False) -> None:
        """Runs the full ingestion pipeline: products -> session -> themes -> details.

        `product_ids` and `limit` scope Phase 4 (details) to a subset of
        products; both are mutually exclusive and default to the full catalog.
        `force` bypasses the landing gate during Phase 4, fetching every
